...
### CiteSeer / PubMed 无法在线下载时

Cora 和 CS 可正常从库内拉取，CiteSeer 和 PubMed 的原始数据来自 GitHub，若网络受限会失败。可先运行项目自带下载脚本（会补全 `data/<数据集>/raw/` 下缺失文件，支持代理）：
...
若仍失败，可设置代理后重试（例如本机代理为 `http://127.0.0.1:7890`）：
...
或在能访问 GitHub 的机器上执行上述命令，再把 `data/CiteSeer`、`data/PubMed` 拷回本机对应目录。
